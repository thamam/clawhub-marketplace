...
// Helpers
// ---------------------------------------------------------------------------
async function chub(args, json = true) {
...
// Server
// ---------------------------------------------------------------------------
const server = new McpServer({ name: "context-hub", version: "1.0.0" }, {
    instructions: "Context Hub — search and fetch fleet docs, skills, decisions, and patterns. " +
...
// ---------------------------------------------------------------------------
server.tool("chub_search", "Search Context Hub for docs, skills, decisions, or patterns. Returns IDs and summaries.", {
    query: z
        .string()
        .optional()
...
// ---------------------------------------------------------------------------
server.tool("chub_get", "Fetch full content of a doc or skill by ID. Use IDs from chub_search results.", {
...
// ---------------------------------------------------------------------------
server.tool("chub_annotate", "Add, read, or list agent notes on Context Hub entries. Notes persist locally across sessions.", {
    ...
    try {
        ...
        if (list) {
            ...
            if (!id)
                return error("id is required unless using list=true");
            args.push(id);
            if (note)
                args.push(note);
            if (clear)
...
// ---------------------------------------------------------------------------
server.tool("chub_list", "List all available docs and skills in Context Hub.", {
    tags: z
        .string()
        .optional()
...
// ---------------------------------------------------------------------------
server.tool("chub_feedback", "Send quality feedback (thumbs up/down) for a doc or skill to help authors improve content.", {
    ...
    labels: z
        .string()
        .optional()
        .describe("Comma-separated labels: accurate, well-structured, helpful, good-examples, " +
...
// ---------------------------------------------------------------------------
server.resource("registry", "chub://registry", {
